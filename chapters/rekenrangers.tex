...
Personalisation is the property where RekenRangers has the most room for growth. The original intention was to integrate an avatar system into the safari theme, where each child would have their own ranger character that could be customised with outfits and accessories purchased with earned coins. Due to time constraints and the need to prioritise the research-critical features, this was not implemented in the current version of the platform. The avatar gap is therefore one of the limitations discussed in Section~\ref{sec:limitations}, with concrete proposals listed under future work in Section~\ref{sec:future-work}.

What is in place are the choices the platform gives the child. When enabled by the teacher or researcher, children can choose for themselves which operation they want to practise, what difficulty level they prefer, and whether they want to see a timer. As discussed in Section~\ref{sec:exercise-properties}, this kind of choice can support a sense of ownership. Each child also works with their own account, so that their coin balance and puzzle progress are personal to them, and a child who has saved up coins and completed three animal puzzles sees a different puzzle screen than a classmate who just started. This gives some sense of individual identity within the platform, even without an avatar.

\subsection{Accessibility}
RekenRangers is a web application hosted on a KU Leuven server and accessible at \texttt{https://rekenrangers.cs.kuleuven.be/}. It runs in any modern browser without requiring software installation, plugins, or account creation through an external service. This means the tool works on the laptops, tablets, Chromebooks, and even phones that schools and families already have. The only requirements are an internet connection and a browser. There is currently no dedicated mobile app, and the interface is not specifically optimised for smaller screens, but both are realistic extensions for future development.

Access to the platform is managed through a two-step login system designed to be usable by young children without adult help. The child first enters a group code, which identifies their class or research group. They then select their name from a list and enter a personal animal code: a sequence of four animal icons that serves as a child-friendly alternative to a traditional password.
...
\begin{figure}
    ...
    \begin{subfigure}[t]{\textwidth}
        ...
        \caption{Student login screen. After selecting their name, the child enters a personal animal code by clicking four animal icons in the correct order.}
    ...
    \begin{subfigure}[t]{\textwidth}
        ...
        \caption{Printable login cards. Each card shows the child's name, group code, and personal animal code.}
        \label{fig:login-cards}
    \end{subfigure}
    \caption{The RekenRangers login system.}
    \label{fig:accessibility}
\end{figure}

Teachers and researchers generate these cards through the group management screen in the teacher dashboard. The cards can be printed and handed out in class, and since they contain everything a child needs to log in, they also enable use at home. Figure~\ref{fig:accessibility} shows the login screen and the printable cards together. This low-threshold approach to access means that RekenRangers can serve as both a classroom tool and a home practice tool without any additional setup.
...
One specific design decision worth mentioning is that the entire child-facing interface can be operated using only the keyboard. Answers are typed in directly, smileys are selected with the f, g, h, j keys, and puzzle pieces are bought with the spacebar. This choice was made in consultation with Elien Bellon, and is functionally closer to a piece of experimental control than to a usability feature: it ensures that when research mode is active and the mouse is disabled, every interaction the child needs is still reachable.

Across all of these choices, the goal is the one identified in Section \ref{sec:exercise-properties}: an interface that stays out of the child's way, so that the cognitive effort the child spends is spent on the arithmetic.