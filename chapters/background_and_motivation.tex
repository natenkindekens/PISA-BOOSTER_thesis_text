...
With a clear picture of what a good research tool needs to do, the next step is to look at what researchers actually have on their desks today. This section walks through the two tools that came up most often during discussions with the research team at KU Leuven: OpenSesame \cite{mathot_opensesame_2012}, which my co-supervisor used during her own doctoral work (we will look at an example in Section \ref{sec:bellon-studies}), and PsychoPy \cite{peirce2019psychopy2}, which is taught to psychology students. The discussion examines how each of them fares against the six properties introduced in Section \ref{sec:research-properties}. Both tools have been carefully designed and have earned their place in the field. The point of this section is not to argue that they are bad tools; it is to show that even the best tools currently available leave a specific kind of gap, and that this gap becomes critical the moment a study leaves the controlled lab and moves into a classroom.
...
The OpenSesame team has recently added \textit{Sigmund} \cite{sigmundai}, a generative-AI assistant designed to help researchers build experiments, and the project itself continues to evolve, for example with OSWeb for online studies \cite{mathot2022osweb}. These are real improvements, but they do not change the underlying picture: when a study moves beyond what the GUI covers, programming knowledge is still required. OpenSesame is a powerful and well-designed tool with many possibilities, but for researchers without a programming background it can be limiting.
