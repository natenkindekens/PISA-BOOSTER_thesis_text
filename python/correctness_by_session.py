...
def make_figure(csv_files, title, out_filename):
    ...
    session_avg = df.groupby("session_num")["is_correct"].mean().reindex(session_idx)
    day_avg = df.groupby("day_num")["is_correct"].mean()
...
make_figure(
    ...
    title="Average Correctness per Session and per Day — No Adaptivity (Conditions 1 & 2)",
    out_filename="correctness_by_session_no_adaptivity.png",
)

make_figure(
    csv_files=["data/group_Conditie_3.csv", "data/group_Conditie_4.csv"],
    title="Average Correctness per Session and per Day — Adaptivity (Conditions 3 & 4)",
    out_filename="correctness_by_session_adaptivity.png",
)
